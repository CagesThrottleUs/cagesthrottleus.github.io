...
abstract: This document explains the architecture of this blog such that it remains always free
...
<Mermaid 
  caption="Fig 1.1: Github Actions Workflow"
  text={`
flowchart TB
    subgraph MAINFLOW["\`MAIN BRANCH WORKFLOW\`"]
        direction TB
        M1["\`Developer Creates Feature\`"]
        M1 --> M2["\`Feature Branch feat/* | fix/* | chore/*\`"]
        M2 --> M3["\`Local Development\`"]
        M3 --> M4["\`npm run dev - Vite Dev Server\`"]
        M4 --> M5["\`npm test - Vitest + Coverage\`"]
        M5 --> M6["\`npm run lint - ESLint\`"]
        M6 --> M7["\`npm run format:check - Prettier\`"]
        M7 --> M8["\`git push origin branch\`"]
        M8 --> M9["\`Create Pull Request → main\`"]
        
        M9 --> M10["\`PR VALIDATION PIPELINE\`"]
        ...
        M17 --> M18{"\`All checks passed?\`"}
        M18 -->|Fail| M19["\`Fix required\`"]
        M19 --> M3
        M18 -->|Pass| M20["\`Ready to merge\`"]
        
        M20 --> M21["\`Merge to main\`"]
        M21 --> M22["\`DEPLOY PIPELINE\`"]
        M22 --> M23["\`Build Job: Checkout\`"]
        M23 --> M24["\`Setup Node + npm cache\`"]
        M24 --> M25["\`npm ci\`"]
        M25 --> M26["\`Quality checks format → lint → test\`"]
        M26 --> M27["\`vite build - Production\`"]
        M27 --> M28["\`Code Splitting\`"]
        M28 --> M29["\`vendor.js - React, Router\`"]
        M28 --> M30["\`ui.js - Adobe Spectrum\`"]
        M28 --> M31["\`mermaid.js - Lazy loaded\`"]
        M28 --> M32["\`app chunks - Components, Hooks\`"]
        M29 --> M33["\`Upload dist/ artifact\`"]
        M30 --> M33
        M31 --> M33
        M32 --> M33
        M33 --> M34["\`Deploy Job: Deploy to Pages\`"]
        M34 --> M35["\`Live Site cagesthrottleus.github.io\`"]
    end
...
Let us first talk the features that will be present when someone goes to the path <code>/blog</code>. They should be able to search for blog post on titles with a contains logic. They can also filter posts based on start and end date range.
...
<Mermaid 
  caption="Fig 1.2: Generating Manifests"
  text={`
flowchart TB
    ...
    CheckPosts -->|Yes| GetFiles["\`Get all *.mdx files sorted newest first\`"]
    
    GetFiles --> FileLoop{"\`For each MDX file\`"}
    
    FileLoop -->|Process| ExtractMeta["\`Extract metadata get_post_metadata\`"]
    ExtractMeta --> ReadFile["\`Read file content\`"]
    ReadFile --> ParseFront["\`Extract frontmatter extract_frontmatter\`"]
    
    ParseFront --> RegexMatch{"\`Match ---...--- pattern?\`"}
    RegexMatch -->|No| ReturnEmpty["\`Return empty dict\`"]
    RegexMatch -->|Yes| ParseLines["\`Parse key:value pairs Remove quotes & whitespace\`"]
    
    ParseLines --> ExtractSlug["\`Extract slug from filename\`"]
    ExtractSlug --> ParseDate{"\`Date in slug YYYY-MM-DD?\`"}
    ...
    ValidateTitle -->|No| WarnSkip["\`WARN: Missing title Return None\`"]
    ValidateTitle -->|Yes| CreateMeta["\`Create PostMetadata object - slug - title - classification - abstract - publishDate - version - thumbnail\`"]
    ...
    CheckValid -->|Yes| SortPosts["\`Sort posts by publishDate newest first\`"]
    
    SortPosts --> CalcPages["\`Calculate total pages ceil totalPosts / 50\`"]
    CalcPages --> PageLoop{"\`For each page\`"}
    
    PageLoop --> SlicePosts["\`Slice posts array start: page-1 × 50 end: min start+50, total\`"]
    SlicePosts --> CreatePage["\`Create page manifest - page number - posts array\`"]
    CreatePage --> SavePage["\`Save to manifests/page-N.json\`"]
    SavePage --> PageLoop
    
    PageLoop -->|All pages done| CreateIndex["\`Create index manifest - version timestamp - totalPosts - totalPages - postsPerPage: 50 - latestPosts top 10 - pages map\`"]
...
flowchart TD
    ...
    Trigger -->|Push to 'blog' branch with posts/** changes| Setup
    Trigger -->|Manual Dispatch| Setup
    Setup["\`Checkout blog branch\`"] --> Python["\`Set up Python 3.x\`"]
    Python --> Generate["\`Run generate_manifest.py\`"]
    Generate --> Config["\`Configure Git User GitHub Actions Bot\`"]
    Config --> Stage["\`Stage manifests/ directory\`"]
    Stage --> Check{"\`Changes Detected?\`"}
    Check -->|No changes| NoOp["\`Echo: No changes to commit\`"]
    Check -->|Changes present| Commit["\`Commit with message: chore: update blog manifests\`"]
...
<Mermaid text={diagram_3_2} caption="Fig 1.3: Workflow for the blog post" />
...
**CLASSIFICATION**: RESTRICTED // SAP // ORCON 
**DISTRIBUTION**: Originator controlled
